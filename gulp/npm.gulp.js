...
gulp.task('npm:update', ['load:app'], function (finish) {
  ...
  if (!_.get(app, 'config.gulp.npmUpdate')) {
    fancyLog('NPM auto-update is disabled');
    return finish();
  }
  ...
  async()
    ...
    .then(function (next) {
      ...
      fancyLog('Project requires NPM update', ansiColor.grey(`(Package Hash: ${this.git.shortHash}, latest update: ${_.get(this.stamps, ['package.json', 'shortHash']) || 'NONE'})`));
      async()
        .use(asyncExec)

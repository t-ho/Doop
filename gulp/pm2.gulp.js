...
gulp.task('pm2-deploy', ['load:app'], function (finish) {
  async()
    ...
    .then(function (next) {
      ...
      if (proc) {
        fancyLog('Found existing PM2 process', colors.cyan(app.config.name), 'rebooting it');
        pm2.restart(app.config.name, function (err) {
          if (err) return next(err);
          fancyLog(colors.yellow('⟲'), 'PM2 process rebooted');
          next();
        });
      } else {
        fancyLog('No PM2 process named', colors.cyan(app.config.name), 'found. Booting for first time');
        pm2.start({ name: app.config.name, script: paths.root + '/server.js' }, function (err) {
          ...
          if (err == 'Error: Script already launched') {
            fancyLog('PM2 process already exists but without a name. Rebooting + renaming');
            async()
              .then(function (next) {
            ...
            fancyLog(colors.green('✔'), 'PM2 process started');
            next();
          }
          // }}}
        });

...
app.get('/api/session/profile', function (req, res) {
  if (!req.user || (req.user && req.user.status != 'active')) return res.status(200).send({});
...
/**
* Check whether the current user's session is expired or not
*/
app.get('/api/session/alive', function (req, res) {
  if (!req.user || (req.user && req.user.status != 'active')) {
